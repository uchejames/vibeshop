...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const { user, error: authError } = await mockAuth.signUp(email, password, fullName, userType);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4">
      <Card className="w-full max-w-md border-slate-700 bg-slate-800/50 p-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Join Vibeshop</h1>
          <p className="text-slate-400 text-sm mt-2">Start selling or shopping today</p>
        </div>
